...
<tr class="item <?php if (!empty($class)) echo $class; ?>" data-order_item_id="<?php echo $item_id; ?>">
    ...
    <td class="thumb">
        ...
        <?php if ($_product) : ?>
            <?php if ($item["wcla_design_id"]): ?>				
                <?php
                $design_data = $WCLA::load_design_data($item["wcla_design_id"]);
                $design = json_decode(stripslashes_deep($design_data->data));
                $designInfo=  unserialize($design_data->info);
               
                $output = '';
                foreach ($design->data->locations as $location) {
                    //$output.=$location->svg;
                    //$output.='<img src="' . LIVEART_DESIGNS_URL . 'image_' . $values["design_id"] . '_' . $location->name . '_.svg">';
                    $output.='<embed style="width:100px;height:100px;max-width:587px;padding:0;margin:0;" src="' . DESIGNER_DESIGNS_URL .$item["wcla_design_id"].'/'. $designInfo["images"][$location->name]["svg"] .'" type="image/svg+xml" pluginspage="http://www.adobe.com/svg/viewer/install/"></embed><a  href="' . DESIGNER_DESIGNS_URL .$item["wcla_design_id"].'/'. $designInfo["images"][$location->name]["svg"] .'" target="_blank">Preview</a>';
                }
                echo $output;
                ?>
            <?php else: ?>	
    ...
    <td class="name">
        ...
        <?php if (isset($design_data)): ?>
            <?php if ($design->data->namesNumbers): ?>
                <table class="table-grid wcla-grid">
                    <thead>
                        <tr>
                            <td>Name</td>
                            <td>Number</td>
                    ...
                    <tbody>
                        <?php foreach ($design->data->namesNumbers as $row): ?>
                            <tr>
        ...
        <?php if (isset($design_data)): ?>
            <?php if ($design->data->quantities): ?>
                <table class="table-grid wcla-grid">
                    ...
                    <tbody>
                        <?php foreach ($design->data->quantities as $row): ?>
                            <tr>
                                <td><?php echo $row->size ?></td>
